...
**~16 done · ~10 partial · ~16 not started** _(recounted 2026-07-09 at the submission-push close-out: §4A1–8 all ✅ — the full battery ran in NB07 Steps 17–23 (side-level ΔBIC −110 closed as skew-fitting; taxometrics CCFI .151 dimensional; power 100%/0% on the deciding pair; 288/288 multiverse specs unimodal); §2.1/§2.2 ✅ (the two reviewer-facing soft spots the battery existed to close); §4C12 ✅ (gold suite complete: allowlist P .842/R .917, classifiers, repetition de-conflation); §4E-a ✅ (overlap split, gold-adjudicated Trusted); §4E-g 🟡 (gold quotation rate in the panel). Done now leads the count — the remaining ⬜ mass is the §4B/§4C11/§4D positive-story program and §4F replication, deliberately out of the submission-minimum scope.)_
...
- **NB06 delivered the first real distributional rigor** at the *corrected caller-level unit*:
  ...
  finding and the robust gender effect are unchanged. (The "robust education" effect did not
  survive the 2026-07-19 Welch-ANOVA upgrade in NB07 Step 8 — it was the n=6 unknown-education
  bucket under pooled variance; a suggestive monotonic trend now.) It explicitly defers the
  "airtight" battery (Silverman / bootstrap-LRT / skew-fit / taxometrics).
- **The airtight battery is done (2026-07-09, NB07 Steps 14–23):** dip/Silverman/BLRT/fit-family
  at every unit, four-matrix clusterability, taxometrics (the centerpiece), recovery power, the
  288-spec multiverse, and the NXT gold suite that measures every heuristic the analysis rides.
  **Still ahead:** the "positive story" — trait stability/ICC, CFA, mixed-effects demographics,
  accommodation, style-mismatch → call quality (§4B10, §4C11, §4D) — and external replication
  (§4F). This matches the audit's own §5 ordering: fixes → battery → positive story → features
  → replication.
...
_**Addendum 2026-07-09:** the "single most important thing" is done. NB07 Steps 14–23 (submission plan T6–T15) converted the null into formal, positive evidence: dip + Silverman + BLRT + fit-family at every unit of analysis; the side-level ΔBIC −110 formally explained as skew-fitting (skewnorm over GMM(2) by 36); four-matrix clusterability at gap k̂=1; taxometrics at CCFI .151 (dimensional, π-robust) on the caller indicators; recovery power 100% / false-alarm 0% on the deciding pair; 288/288 multiverse specifications unimodal; and a gold suite that measures every heuristic the analysis rides (allowlist P .842/R .917; coop/obstr overlap validated; repetition de-conflated; questions measured-and-excluded; a gold involvement axis from human-annotated behaviors showing the same dimensional shape). The claim is **dimensional, not normal**. What remains for "airtight" is the §4B10 reliability line and the positive-story program — the next paper, not this one._
...
1. ✅ **No formal multimodality test anywhere.** Unimodality claims rest on KDE mode counts with ad-hoc prominence thresholds that drift between notebooks (1% in NB3/NB4, 5% in NB5).
   ↳ **Status:** ✅ Closed 2026-07-09. The accepted pair runs everywhere: dip + Silverman (+ BLRT + fit-family) at caller, side, and utterance level, on the volume PCs, F_int, and the gold involvement axis — all rows in NB07 Step 23's single battery table with fixed seeds and printed B. KDE mode counting survives only as the recorded Step-5 descriptive convention (fixed 5% prominence), never as the test.
2. ✅ **The speaker-level BIC picked k=2** (15,756 vs 15,866 for k=1). You argued ΔBIC −110 ≪ the backchannel split's −53,000, which is fair rhetoric but not a test. A 2-Gaussian fit beating 1 Gaussian is *expected* for any skewed unimodal distribution — but you never fit the one-component skewed alternative that would prove it.
   ↳ **Status:** ✅ Closed 2026-07-09. Both halves now stand: caller-level dedup flips the result empirically (ΔBIC(2−1) = +11, k=1), **and the one-component skewed alternative is fitted at the side level itself — skewnorm beats the 2-Gaussian mixture by ΔBIC 36** (NB07 Step 18), so the old −110 is formally explained as skew-fitting, not types. The same fit-family resolution holds at every other battery cell, and Step 21 shows the pair's error profile is 100% detection / 0% false alarm at our n.
...
8. **Smaller items:** the 38-token backchannel allowlist is copy-pasted across three notebooks with no sensitivity check; NB4/NB5 skip the winsorize+log1p step NB3 Part C established; no parallel analysis for component retention; 17,053 pitch-null rows silently dropped; `tannen_features.csv` — the file named for the initiative — is never read by any notebook.
   ↳ **Status (updated 2026-07-09):** ✅ parallel analysis (Horn's, NB06/NB07). ✅ **allowlist sensitivity** — measured against gold (P .842 / R .917) and swept as a six-definition multiverse axis (NB07 Steps 12/15/22). ✅ **winsorize+log1p** — discharged inside the multiverse transform arm (and the Step-18 tail diagnosis). 🟡 pitch nulls: the vol11+pitch3 multiverse arms document the sensitivity (all unimodal), but the 8%-null caller-mean handling itself is still just NaN-skipping. ⬜ `tannen_features.csv` is still never read by a notebook.
...
3. 🟡 **Degenerate features.** `personal_focus_score`: 54.7% NaN and 64.6% of defined values exactly 0.0 or 1.0 — Empath saturates on short utterances; aggregate to speaker level over content words, or replace with a pronoun+lexicon score over a minimum-token window. `rising_terminal`: 68.5% missing **not at random** (short/unvoiced tails), and `build_merge_test.py` fills NaN→0, conflating "unmeasurable" with "not rising."
   ↳ **Status:** Both extractor halves fixed and now **adjudicated in NB07 Step 11 (2026-07-09, submission-plan T3)**: pooled personal focus is the sanctioned estimator with printed evidence (caller-level PF_ratio at ≥30 pooled hits — 100% coverage of the 487 callers, mean .619 ± .129, zero 0/1 saturation vs the per-utterance score's 71.5% null / 48.6% saturation, recomputed inline as the contrast) and the degenerate per-utterance `Personal Focus Score` is **Deprecated** in the registry; rising-terminal missingness audited — 30.3% null, MNAR but **voicing/length-dependent** (null RISES with utterance length, 21%→35% for ≤2→≥6 tokens, contra the docstring's short-tails guess), slope clip ±1000 Hz/s recorded, both RT columns Trusted with never-fillna(0) notes. _In use 2026-07-09 (NB07 Step 16, T8): `PF_ratio` (floor 30, 100% coverage) and `rt_rising_share` (defined-only mean, MNAR caveat carried) are involvement-panel variables — both load ≈+0.35 on the engagement factor._ _(2026-07-29 **semitone redesign + gold validation** — `rising_terminal.py`, evidence in `analysis/validate_rising_terminal.py`: the flag now thresholds the semitone slope (new Trusted column `Terminal ST Slope`) at 3.07 st/s — the old 30 Hz/s operating point at the corpus-median 169 Hz register — because the Hz rule demanded ~1.7× steeper proportional rises from low registers, mechanically coupling share to pitch: side-level r(share, pitch) .485→.315 after the fix, sex gap in share .119→.071, 2,934 utts reclassified 0→1 / 1,372 1→0 (97.1% agreement), null set + Hz-slope column bit-identical, window/gates unchanged. Gold-DA known-groups: rising share .616 yn-questions vs .248 statements (2.48×, Welch p 6e-122, AUC(ST slope) .721) **PASS**; declarative-q `^d` .410 (1.65×, p 2.5e-08) — the first-run 2× arm FAILED and is adjudicated on record (only ~40–60% of conversational declarative questions rise; DAMSL `^d` partly context-labeled); admission rests on the qy arm. MNAR carried, unchanged by design: defined-rate–pitch r +.352 (F .719 / M .591) — sanctioned caller estimator = pooled defined-only share over all 487 (tails min 2 / median 154) + `rt_defined_share` coverage companion, ≥30-tail subset (428) as S8 robustness, and NB08 App-A gains a PC×coverage artifact gate. MGQ recomposed on the new flag (2,436→2,406 flagged). Registry now 46 Trusted.)_ Residual for ✅: the C2 FROZEN banner on `scripts/build_merge_test.py`, whose line-102 `fillna(0)` is the sole remaining NaN→0 conflation (replication-only).
...
6. ✅ **Taxometric analysis — the centerpiece I'd recommend.** Types-vs-continuum is a solved methodological problem in psychopathology research: Meehl's MAXEIG/MAMBAC/L-Mode with the **Comparison Curve Fit Index** (CCFI < 0.45 ⇒ dimensional; `RTaxometrics` in R; MAMBAC is simple enough to hand-roll in Python). Nobody in the conversational-style literature has applied taxometrics. With 543 callers (or 3,595 sides) and your indicator set, this is feasible and would be a publishable methods contribution on its own: "Conversational style is dimensional, not taxonic."
   ↳ **Status:** ✅ Done 2026-07-09 (T12): `src/swb_extract/taxometrics.py` (gen_data / MAMBAC / MAXEIG / L-Mode / CCFI; 9 known-answer tests incl. the end-to-end pair — dimensional-generated → CCFI < .45, taxonic at the paper's π=.45/d≈2.9 → CCFI > .55) + NB07 Step 20. **Caller vol11 indicators (6, by |PC1 loading|, |r|≤.85 dedup): CCFI mambac .106 / maxeig .267 / lmode .079, mean .151 → decisively DIMENSIONAL**, robust to base rate (π .30/.45/.55 → .136/.151/.135); GMM(2) weights .40/.60 vs the paper's claimed .45/.55. Gold-involvement variant (side, n=1,085): mean CCFI .460 → **ambiguous per the pre-registered bands** — with the recorded validity caveat that the sparse per-100 behavior rates carry mean inter-indicator r of only .09, too little shared signal for taxometric discrimination (low indicator validity, not evidence for types). Envelope figure (the publishable exhibit) + RTaxometrics CSV exports in place. The marker's own framing is realized: "conversational style is dimensional, not taxonic" is now a formal, tested claim._
7. ✅ **Recovery simulation (power analysis for types).** Simulate data with the paper's own claimed structure (means −0.31/+0.36, SDs 0.13/0.31, weights 0.55/0.45), push it through your corrected pipeline, show dip/BLRT/taxometrics all detect it. This converts your null into "we had the power to see two styles; they are not there."
   ↳ **Status:** ✅ Done 2026-07-09 (NB07 Step 21, T13; B=200 per arm, decision rules fixed in advance; BLRT inner B reduced 199→99/49 per the budget clause, printed). **Detection of the paper's mixture at n=487: dip 10% / Silverman 82% / BLRT 100% / fit-family 100%** (side n=3,705: 89/100/100/100 — the plan's nominal 4,876 predates the manifest count). **False alarms on the matched skew-normal continuum: dip 0% / Silverman 0% / BLRT 76% / fit-family 0%** — BLRT-alone alarms on skew by design, which is exactly why the battery pairs it with fit-family. **Taxometric sub-arm on our exact indicator basis: CCFI detection 100%, false-alarm 0%** (B=25/side, inner comparison B=50, disjoint seeds). The deciding instruments run at 100% power / 0% false alarm at our n.
8. ✅ **Multiverse / specification-curve analysis.** Your conclusion currently depends on one backchannel definition (38-token allowlist), one transform (winsorize+log1p), one aggregation (mean over ≥20 utterances). Run the grid — backchannel definition (allowlist / token-count threshold / DAMSL-trained classifier), scaling, feature subsets, with/without pitch, level — and report the fraction of specifications yielding unimodality. This is what "exhaustive" looks like to a reviewer.
   ↳ **Status:** ✅ Done 2026-07-09 (NB07 Step 22, T14). Grid: bc_def {allowlist38, token≤2, union, none, classifier (Step-15-admitted), gold-subset} × transform {none, winsor+log1p} × unit {caller, side} × min_utt {10,20,40} × feature_set {vol11, vol11+pitch3, vol+interactional, vol+int+panel} = **288 specifications — 288/288 unimodal** (dip p>.05 AND a single KDE mode at bw .25/.40/.60 by the notebook's standing Step-5 instrument, 5% prominence; the strict-floor `count_modes` value recorded alongside — an instrument-alignment decision made after a stub-mask smoke run exposed the floor counter reading far-tail micro-bumps as modes, before the real grid was read). ΔBIC sign favors k=1 in 186/288; the k=2 residue is the skew-fitting pattern Step 18 formally explained. Gold-subset arms carry reduced n, printed (caller 248–320, side 416–1,081). Specification-curve figure in NB07; nothing to enumerate — no non-unimodal specs. The §2.8 winsorize+log1p and allowlist-sensitivity items are discharged inside this frame.
...
9. 🟡 **Caller-level deduplication** via `call_con_tab.csv` → caller_no. Rerun the distributional battery at true speaker level (543 callers). All demographic claims need re-estimation as **mixed-effects models**: `feature ~ sex + region + generation + education + topic + (1|caller)`, with FDR correction (statsmodels `MixedLM` / `pymer4`). This will also properly adjudicate NYC-vs-rest (currently confoundable with gender/generation composition) and give the gender–F3 claim its missing test.
   ↳ **Status:** Caller dedup done (NB04/05/06) and FDR added (NB06). The mixed-effects models themselves — `MixedLM` with `(1|caller)` and topic as a covariate — are ⬜; NB07 uses Welch t / **Welch ANOVA** on caller means (`equal_var=False`, upgraded 2026-07-19 from classic ANOVA — the Fisher education "effect" did not survive the upgrade: NB07 Step 8b).
...
11. ⬜ **Confirmatory factor analysis** (`semopy`). Tannen predicts the HI markers covary: short FTOs, latching, overlap, speed, loudness, questions, personal focus. Your NB5 varimax already shows volume, loudness, rate, and interaction splitting into *separate* factors — i.e., a single HI–HC axis may not exist as a unidimensional construct. Test a hierarchical model (general involvement factor over the four group factors) vs orthogonal factors and report fit. Either result reshapes the paper: a continuum needs an axis, and right now your best candidate is F3, not PC0.
    ↳ **Status:** Not started (no confirmatory model). NB05 has *exploratory* varimax (F0 loudness / F1 rate / F2 overlap / F3 interactional) — suggestive, but no confirmatory model fit / comparison. _Exploratory update 2026-07-29 (**NB07 Step 24**, run in the notebook — in use): PCA on the **maximal trusted caller-level space** — vol11 ∪ F_int set ∪ panel shares ∪ pitch = 26 features, 487/487 complete — retains **K=5** (Horn's, 73.4% cum.): volume keeps its own axis (PC2 20.7%, the old vol-PC1 pattern), within-pause-vs-rate (PC3 13.5%) and loudness (PC4 10.1%) stay separate, and **pitch fuses with the overlap/engagement block on PC1** (22.7%: +pitch/+rt_rising/+overlap vs −pauses/−fillers/−repetitions/−FTO). Replicates NB05's split-factor picture on the canonical table — further exploratory evidence against a unidimensional HI–HC axis. Step 24b overlays all 4 demographics on all 5 PCs; the large PC1 gender split is f0 dimorphism (pitch loads .26–.29), not style — style-flavored gender effects stay moderate on PC2/PC3 (~±0.45). The semopy hierarchical-vs-orthogonal comparison remains the open item._
...
- ✅ **(b) Laughter counter** (map #17, trivial, dim 9 currently empty). _(Done: `laughter.py` — see §3.5; merged, Trusted, and in use — `laughs_per_100utt` is an F_int + panel variable as of NB07 Step 16.)_
...
2. ✅ **The unimodality battery** (1–2 weeks): dip + Silverman + bootstrap-LRT + skew-fit comparison + multivariate clusterability + recovery simulation + multiverse grid. This is the defensible core of the "continuum" paper.
   ↳ **All ✅ 2026-07-09 (NB07 Steps 17–23):** dip + parallel analysis · Silverman / BLRT / skew-fit run at every unit (side-level ΔBIC −110 closed: skewnorm over GMM2 by 36) · multivariate (4 matrices, gap k̂=1) · taxometrics (caller CCFI .151 dimensional; gold variant ambiguous-on-low-validity) · recovery (deciding pair 100%/0%) · multiverse (288/288 unimodal). The defensible core is built.
...
- **NB07 state.** Loads via `load_features_table(include="provisional")` (cell 3); masks FTO to
  ...
  2026-06-30: K=2, PC1 42.5%, dip p .994/.993, ΔBIC(2−1)=+11, gender robust,
  region/generation null; education demoted to a suggestive trend by the 2026-07-19
  Welch-ANOVA upgrade, Step 8b). **Three of its eleven live features are WIP** (the two repetition
